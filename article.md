...
> Big thanks to [Tobias Koppers](https://twitter.com/wSokra), founder of webpack, for answering several questions about this topic and for proofreading this article.
...
Interestingly, Module Federation's behavior is a bit different when it comes to do Dynamic Module Federation. The reason is that dynamic remotes are not known at program start, and hence Module Federation cannot draw their versions into consideration during its initialization phase.
...
In this case, Module Federation will decide for a shared version and for possible fallbacks per scope.

For this, let's assume we have two more micro frontends, called MFE2 and MFE3:
...
As we don't specify a scope for them, Module Federation uses the ``default`` here.

When starting the application, we see that Module Federation decides for one version within the ``default`` scope and for an other version within the ``other`` scope. In both cases, the usual rules described in this article are applied. Hence, we get version ``1.0.1`` for the shell and MFE1:
